...
import {
  ...
  TrendingUp, CheckCircle, Clock, Sparkles, LogOut, X, AlertTriangle,
  Calendar, Mail, Link2
} from "lucide-react";
...
function OverviewTab({ stats, loading }: { stats: Stats | null; loading: boolean }) {
  ...
  if (!stats) return <p className="text-white/50 text-center py-10">Không thể tải dữ liệu.</p>;
...
const TABS = [
  ...
  { id: "cards", label: "Thiệp", icon: CreditCard },
...
export default function AdminPage() {
  ...
  return (
    ...
    >
      {/* Top Bar */}
      <header className="border-b border-white/8 bg-black/40 backdrop-blur-md px-4 sm:px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-white/40 text-xs">HPBD • Birthday Card System</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={refreshing}
            className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 flex items-center justify-center text-white/50 hover:text-white transition cursor-pointer"
            title="Làm mới dữ liệu"
          >
            <RefreshCw className={`w-4 h-4 ${refreshing ? "animate-spin" : ""}`} />
          </button>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10">
            <div className="w-5 h-5 rounded-full bg-gradient-to-br from-pink-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold">
              {user?.email?.[0]?.toUpperCase() ?? "A"}
            </div>
            <span className="text-white/60 text-xs max-w-[140px] truncate">{user?.email}</span>
          </div>
          <button
            onClick={logout}
            className="w-8 h-8 rounded-lg bg-white/5 hover:bg-red-500/20 flex items-center justify-center text-white/50 hover:text-red-400 transition cursor-pointer"
            title="Đăng xuất"
          >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        {/* Stats Mini Bar (always visible) */}
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
            {[
              { label: "Thiệp", value: stats.totalCards, icon: CreditCard, color: "#f472b6" },
              { label: "Lời chúc", value: stats.totalWishes, icon: MessageSquare, color: "#a78bfa" },
              { label: "Đã mở", value: stats.revealedCards, icon: CheckCircle, color: "#34d399" },
              { label: "Chờ mở", value: stats.pendingCards, icon: Clock, color: "#fbbf24" },
        ...
        {/* Tabs */}
        <div className="flex gap-1 mb-6 bg-white/5 p-1 rounded-xl border border-white/8 w-fit">
          {TABS.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition cursor-pointer ${
                activeTab === id
                  ? "bg-gradient-to-r from-pink-500/30 to-purple-500/30 text-white border border-white/15"
